...
הפיילוט בעיתונות הערבית החל בשתי שאילתות שתרגמו את שמות היחידות, `الفرق الليلية`{lang=ar dir=rtl} ו־`فصائل الميدان`{lang=ar dir=rtl}. שתיהן לא החזירו התאמות, וגם שלושה כתיבים אחרים של וינגייט לא הניבו תוצאה. יומן החיפוש שמר את הכישלונות; תוצאת אפס מעידה רק על החיפוש במאגר ובטווח התאריכים שנבדקו, ולא על היעדר המונח מן העיתונות. בעקבות זאת הרחבתי את מטריצת החיפוש: שמות וכתיבים חלופיים; חניתה ודבוריה; צינור הנפט; וכן פעולות ותוצאות ובהן סיור, מארב, ירי, נפגעים, כיתור וחיפוש. הכתיב `ونجيت`{lang=ar dir=rtl} העלה 2 תוצאות, והצירוף `الكابتن ونجيت`{lang=ar dir=rtl} הוביל היישר לדיווח על דבוריה.[^5]

מילה משותפת לבדה אינה מחברת שני דיווחים לאירוע אחד. לשם כך דרשתי חפיפה בתאריך ובמקום, ועוד שני מאפיינים תואמים לפחות. בדבוריה התאימו גם הכוח הבריטי־יהודי המעורב, פציעת וינגייט, מאזן הנפגעים והפעולה בכפר לאחר הקרב. כאשר פרט מכריע היה חסר, סיווגתי את ההתאמה כ„סבירה” ולא כמאומתת. כך נעשה האירוע ליחידת ההשוואה; מילת המפתח נותרה אמצעי ניווט בלבד.

## 3. בין העמוד לטענה {#בין-העמוד-לטענה}

תוצאה שעלתה בחיפוש היא מקור אפשרי לבדיקה, ועדיין אינה ראיה. בכל רשומה שמרתי שש שכבות, וכל אחת מתעדת פעולה אחרת. בשכבה הראשונה שמרתי את נתוני הארכיון: העיתון, התאריך והקישור. בשנייה שמרתי פלט OCR גולמי שהפיקה המכונה. בשכבה השלישית מופיע התמלול שקראתי מן הסריקה; ברביעית, האחדת כתיב זהירה של כותרת משובשת או כתיב היסטורי; בחמישית, התרגום; ורק בשישית, המסקנה המחקרית. אלה אינן גרסאות מתחרות. ההפרדה מבהירה מה סיפק הארכיון, מה ניחשה המכונה, מה קרא החוקר ומה הסיק.

גם המאגר עצמו מחייב שקיפות, משום שבמחקר כזה פועלים שני מנגנוני בחירה בזה אחר זה. האוסף הדיגיטלי כולל רק חלק מן העיתונים ששרדו. לאחר מכן השאילתה ואיכות ה־OCR מצמצמות שוב את החומר שמופיע ברשימת התוצאות. מחקרים על הטיות באוספים, על תהליכי עבודה בין־תחומיים ועל ממשק impresso מצביעים כולם על דרישה אחת: לחשוף את הרכב האוסף, את מקור הנתונים ואת שרשרת העיבוד, את שלבי העיבוד ואת האיכות של כל שלב. מאגר דיגיטלי ורשימת תוצאות אינם עותק מלא ושקוף של העיתונות ההיסטורית.[^6]
...
## 4. מן הפיילוט ליחידת עבודה לשימוש חוזר {#מן-הפיילוט-לסקיל}
...
```mermaid
flowchart TD
    A["שאלה ממוקדת<br/>הגדירו את הטענה, טווח התאריכים, השפות ונקודות המבט."]
    B["מטריצת חיפוש רב־לשונית<br/>שלבו שמות, כתיבים, מקומות, פעולות ותוצאות."]
    C["איתור מקורות אפשריים לבדיקה<br/>תעדו כל שאילתה, לרבות חיפושים שלא הניבו תוצאות."]
    D["רשימת מקורות מתועדת<br/>שמרו מזהים, מטא־דאטה, קישורים, מצב אימות ומגבלות זכויות."]
    E["פלט OCR גולמי<br/>הפרידו בין פלט המכונה לבין התיקונים המוצעים."]
    F["אימות חזותי<br/>בדקו את הסריקה לפני אישור נוסח או פרט."]
    G["התאמה בין דיווחים<br/>דרשו התאמה בתאריך, במקום ולפחות בשני מאפיינים נוספים."]
    H["הערכת מקורות<br/>בחנו עצמאות, סתירות ומסגור."]
    I["נקודת בקרה אנושית<br/>אשרו, דחו או החזירו את המקור לבדיקה נוספת."]
    J["חבילת ראיות מתועדת<br/>הפיקו מראי מקום, טבלת התאמה וסיכום מסויג."]
    A --> B
    B --> C
    C --> D
    D --> E
...
*איור 2. משאלה ממוקדת לחבילת ראיות מתועדת שאפשר לבדוק. מקור שנדחה חוזר ליומן החיפוש ונשאר בתיעוד ההחלטות. נקודת הבקרה האנושית היא החלטה מחקרית, לא בדיקה סופית למראית עין.*
...
גם בקידוד בסיוע מודל חלוקת העבודה נשארת ברורה. המודל רשאי להציע אם מקור המידע הוא הודעה רשמית, כתב מקומי או שילוב מקורות בידי מערכת העיתון, ולדרג מקורות אפשריים לבדיקה לפי רלוונטיות. החוקר מגדיר את הקטגוריות ובוחן אותן מול מדגם שקודד בידי חוקר. ASReview הוא כלי לסינון ספרות מחקרית באמצעות למידה פעילה (active learning). האלגוריתם מתעדף רשומות, אך החוקר מחליט מה לכלול ומתי לעצור. יחידת העבודה אינה מיישמת במלואה את שיטת הקידוד או את ASReview; היא מאמצת רק את חלוקת העבודה שהדוגמאות האלה ממחישות. המכונה מקצרת את התור ושומרת את תיעוד ההחלטות, והחוקר מאמת, מפרש ומצטט.[^9]

## 5. מה השיטה מוסיפה {#מה-השיטה-מוסיפה}

התרומה אינה אלגוריתם חדש, וגם לא טענה לחידוש בכל אחד מן המרכיבים. היא מציעה לחוקר יחיד דרך פשוטה ונגישה לחבר בין דרכי עבודה שכבר מוכרות בכמה קהילות מחקר: חיפוש בכמה שפות, ביקורת על גבולות האוסף, תעדוף מקורות אפשריים לבדיקה, קידוד בסיוע מודל ובדיקת מקורות. האירוע ההיסטורי מארגן את הפעולות האלה סביב שאלה אחת. התוצאה היא חבילת ראיות מתועדת שאפשר לבדוק, לתקן ולהעביר לחוקר אחר. הדבר מועיל במיוחד כאשר קבוצות יריבות מתארות אותה פעולה במונחים שונים.
...
[^2]: יומן החיפוש של פיילוט העיתונות הערבית, 31 ביולי 2026, שאילתות `"الفرق الليلية"`{lang=ar dir=rtl}, `"فصائل الميدان"`{lang=ar dir=rtl}, `ونجيت`{lang=ar dir=rtl}, `"الكابتن ونجيت"`{lang=ar dir=rtl} ו־`"دبورية"`{lang=ar dir=rtl}; תוצאות החיפוש נשמרו כחלק מחבילת המחקר. תוצאת אפס מתעדת את ביצוע השאילתה, לא את היעדר המונח מן העיתונות כולה.
